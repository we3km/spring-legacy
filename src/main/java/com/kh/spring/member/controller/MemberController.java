...
public class MemberController {
	...
	public String loginMember() {
		...
		return "member/login"; // forward 할 jsp의 경로
...
//	}
	...
	// @RequestParam 어노테이션
	// servlet의 request.getParmeter("키")로 봅은 역할을 대신 수행해주는 어노테이션
...
//	}
	...
	/*
	 * 커멘드 객체 방식
	 *  메소드의 매개변수로 VO 클래스 타입을 지정하는 경우, 요청 전달값의 name 속성과 일치하는
	 *  VO 클래스 필드의 Setter 함수를 호출하여 바인딩
	 *  
...
//		 * 응답데이터를 담을 수 있는 객체
//		 * 1) Model
//		 *  - 포워딩할 응답 뷰 페이지로 전달하고자하는 데이터를 맵형식으로 담을 수 있는 객체
//		 *   - 기본 requestScope에 설정을 통해 session Scope에도 데이터를 담을 수 있다.
//		 *   - 클래스 선언부에 @SessionAttribute를 추가하면 데이터가 session Scope 저장
//		 * 
//		 * 
//		 * 2) ModelAndView
...
//		 * modelAndView는 데이터 추가시 addObject()를 사용하며 view 설정시 setViewName()를 사용
//		 * 
//		 */
//		
...
//	}
	...
	public ModelAndView login(
		...
		if(loginUser != null) {
			...
			/*
			 * RedirectAttributes의 FlashAttribute는 데이터를 session scope에 담고
			 * 리다이렉트가 완료되면 세션 스코프에 있던 데이터를 rquestScope로 변경
			 * 
			 */
...
//		session.invalidate(); // 세션 내부의 모든 데이터를 삭제
	...
	public String insertMember(Member m,
		...
		if(result > 0) {
			...
			model.addAttribute("errorMsg", "회원가입 실패ㅜㅜ");
...
//	}
	...
	public String idCheck(String userId) {
		...
		// 경로가 아닌 값 자체를 반환시키기 위해서는 @ResponsBody가 필요하다
		return result+""; //1, 0 => /WEB-INF/view/0.jsp
...
//		
//		//jackson-databind를 활용하여, VO 클래스, ArrayList, map 데이터를 자동으로 JSON 형태로 바인딩하기
//		
//		return map;
//	}
